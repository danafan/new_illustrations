		...
		<el-card class="box_card" id="box_card">
			<el-form :inline="true" size="mini" class="demo-form-inline" id="el_form">
				...
				<el-form-item label="店铺：">
					<el-select v-model="shop_code" clearable :popper-append-to-body="false" multiple filterable collapse-tags placeholder="全部">
						<el-option v-for="item in store_list" :key="item.shop_code" :label="item.shop_name" :value="item.shop_code">
						</el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="日期:">
					<el-date-picker v-model="dates" type="daterange" unlink-panels value-format="yyyy-MM-dd" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :append-to-body="false" :picker-options="pickerOptions">
					</el-date-picker>
				</el-form-item>
			...
			<el-table size="small" :data="dataObj.data" tooltip-effect="dark" :header-cell-style="{'background':'#f4f4f4'}" :max-height="max_height" v-loading="loading">
				...
				<el-table-column label="操作" align="center" width="200" fixed="right">
					<template slot-scope="scope">
						<el-button class="button_theme" type="text" size="small" @click="$router.push(`/detail?record_id=${scope.row.record_id}`)">查看</el-button>
...
<script>
	...
	export default{
		...
		methods:{
			...
			//点击绑定/同意/拒绝
			setDialog(record_id,type){	//1:绑定;2:同意3:拒绝
				this.record_id = record_id;
				this.type = type;
				if(type == '1'){
					this.dialog_title = '绑定商品编码';
					this.input_placeholder = '必填！每输入一个商品编码要换行（每个商品编码占一行）';
				}else if(type == '2'){
